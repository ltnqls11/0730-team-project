...
import sys
sys.path.append('modules')  # 실제 경로 넣기
...
# --- UI/UX 디자인 컨셉 적용 ---
# 1. 배경: #FAFAFA 또는 #FFFFFF (밝은 무채색 계열) -> #FAFAFA 적용
# 2. 포인트 색: #FF8C42 (따뜻한 오렌지색, 식욕을 자극)
# 3. 폰트: Pretendard, Noto Sans, 또는 기본 sans-serif
# 4. 버튼/입력창: 둥근 모서리 (Rounded), 큰 여백 (Padding) 강조
# 5. 아이콘 사용: 식재료, 냉장고, 장바구니 등 시각적 이해 돕기 (예: lucide-react 아이콘)
...
def apply_custom_css():
    st.markdown(
        ...
        /* 기본 배경색 */
        body {
            background-color: #FAFAFA; /* 밝은 무채색 */
        }
        .stApp {
            background-color: #FAFAFA; /* Streamlit 앱의 배경색 */
        }

        /* 폰트 적용 - Noto Sans KR 우선, Pretendard는 로컬 설치 필요 또는 CDN */
        /* Pretendard 폰트를 사용하려면 해당 폰트 파일이 웹 접근 가능한 곳에 있거나,
           사용자 시스템에 설치되어 있어야 합니다. 여기서는 Noto Sans KR을 기본으로 하고,
           Pretendard는 주석 처리하여 예시로 남겨둡니다. */
        @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@100..900&display=swap');
        /* @import url('https://cdn.jsdelivr.net/gh/orioncactus/pretendard@v1.3.9/dist/web/static/pretendard.min.css'); */
        
        html, body, [class*="st-"], .streamlit-container {
            font-family: 'Noto Sans KR', sans-serif;
            /* font-family: 'Pretendard', 'Noto Sans KR', sans-serif; */
        }

        /* 포인트 색상 (FF8C42) 및 버튼 스타일 */
        .stButton>button {
            background-color: #FF8C42; /* 따뜻한 오렌지 */
            color: white; /* 버튼 텍스트 색상 */
            border-radius: 12px; /* 둥근 모서리 */
            padding: 10px 20px; /* 큰 여백 */
            border: none;
            box-shadow: 2px 2px 8px rgba(0, 0, 0, 0.2); /* 그림자 추가 */
            transition: all 0.2s ease-in-out;
        }
        .stButton>button:hover {
            background-color: #e67d33; /* 호버 시 약간 어둡게 */
        ...
        /* primary 버튼 스타일 (AI 레시피 생성, 룰렛 돌리기 등) */
        .stButton>button.css-1ht1j8f.edgvbvh10 { /* Streamlit primary button class */
            background-color: #FF8C42;
            color: white;
            border-radius: 12px;
            padding: 12px 25px; /* 더 큰 여백 */
            font-weight: bold;
            border: none;
            box-shadow: 3px 3px 10px rgba(0, 0, 0, 0.25);
            transition: all 0.2s ease-in-out;
        }
        .stButton>button.css-1ht1j8f.edgvbvh10:hover {
        ...
        .stTextArea>div>div>textarea {
            ...
            padding: 8px 12px; /* 여백 */
            box-shadow: inset 1px 1px 3px rgba(0, 0, 0, 0.1); /* 미세한 내부 그림자 */
        }

        /* 확장 가능한 컴포넌트 (expander) 스타일 */
        .streamlit-expanderHeader {
            background-color: #ffeccf; /* 밝은 오렌지 계열 배경 */
            border-radius: 8px;
            padding: 10px 15px;
            margin-top: 10px;
            margin-bottom: 5px;
        ...
        .streamlit-expanderContent {
            background-color: #FFFFFF; /* 내부 흰색 배경 */
            border-left: 3px solid #FF8C42; /* 포인트 색상으로 왼쪽 테두리 */
            padding: 15px;
        ...
        .stTabs [data-baseweb="tab-list"] button[aria-selected="true"] {
            background-color: #FF8C42; /* 선택된 탭 배경색 */
            color: white; /* 선택된 탭 텍스트 색상 */
            border-bottom: 3px solid #e67d33; /* 아래쪽에 포인트 라인 */
        }
        ...
        h2, h3, h4 {
            color: #333;
            border-bottom: 2px solid #FF8C42; /* 포인트 색상 라인 */
            padding-bottom: 5px;
            margin-top: 25px;
            margin-bottom: 15px;
        }
        ...
        /* 타이틀 */
        .css-fg4pbf.e1nzilvr1 { /* Streamlit title class */
            color: #333;
            font-size: 2.5em;
            font-weight: bold;
            text-align: center;
        ...
        .stAlert.info { background-color: #e6f7ff; border-left: 5px solid #0099ff; }
        .stAlert.warning { background-color: #fffbe6; border-left: 5px solid #ffcc00; }
        .stAlert.success { background-color: #f6ffed; border-left: 5px solid #52c41a; }
        .stAlert.error { background-color: #fff0f6; border-left: 5px solid #f5222d; }
