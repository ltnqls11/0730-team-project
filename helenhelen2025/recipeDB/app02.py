...
def show_home_page(db: DatabaseManager):
    ...
    with col1:
        st.markdown("""
        <div style="background: linear-gradient(135deg, #E6B8E3 0%, #C4C4C4 100%); padding: 25px; border-radius: 20px; text-align: center; box-shadow: 0 8px 25px rgba(212, 184, 227, 0.25); border: 1px solid rgba(255,255,255,0.3); transition: all 0.3s ease;">
    ...
    with col2:
        st.markdown("""
        <div style="background: linear-gradient(135deg, #B8D4E3 0%, #C4C4C4 100%); padding: 25px; border-radius: 20px; text-align: center; box-shadow: 0 8px 25px rgba(184, 212, 227, 0.25); border: 1px solid rgba(255,255,255,0.3); transition: all 0.3s ease;">
            <div style="font-size: 3em; margin-bottom: 15px;">📖</div>
    ...
    with col3:
        ...
        color_gradient = "linear-gradient(135deg, #F4C2C2 0%, #FF9B9B 100%)" if expiring_count > 0 else "linear-gradient(135deg, #E6B8E3 0%, #C4C4C4 100%)"
    ...
    if recipe.get('nutrition'):
        ...
        if nutrition.get('calories', 0) > 0:
            ...
            fig = px.pie(
                ...
                title="영양소 비율",
                color_discrete_sequence=['#C4C4C4', '#B8D4E3', '#F4C2C2']
            )
...
def show_analytics_dashboard(db: DatabaseManager):
    ...
    if not ingredients_df.empty:
        ...
        fig = go.Figure(data=[go.Pie(
            ...
            hole=0.6,
            marker_colors=['#C4C4C4', '#B8D4E3', '#F4C2C2', '#E6B8E3', '#C2B8E3', '#F0B8E3'],
    ...
    if not history_df.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers',
            name='요리 횟수',
            line=dict(color='#C4C4C4', width=3),
            marker=dict(size=8)
        ))
    ...
    if not recipes_df.empty:
        ...
        fig = px.pie(
            ...
            names='조리시간',
            title='조리시간별 레시피 분포',
            color_discrete_sequence=['#C4C4C4', '#B8D4E3', '#F4C2C2', '#E6B8E3']
        )
